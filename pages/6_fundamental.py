...
def get_hot_dart_reports():
    ...
    try:
        ...
        return filtered_reports[:5], None # 상위 5개만 가져옴
    ...
    with st.spinner("금융감독원 DART 서버를 털어서 핵심 공시만 골라내는 중... 🕵️‍♂️"):
        ...
        if error:
            ...
            with st.spinner("AI 편집장이 공시 내용을 개미들 언어로 번역하여 보고서를 쓰는 중... ✍️"):
                ...
                [🚨 매우 중요한 작성 규칙]
                1. [어투 종결 강제]: 1~3번 항목의 모든 문장 끝은 무조건 "~함", "~됨", "~했음", "~예상됨", "~필요함" 형식의 개조식으로만 작성하세요.
                2. [기호 사용 완전 통제]: 글 전체에 걸쳐서 별표(*) 기호와 이모티콘(이모지)은 단 한 개도 절대 사용하지 마세요. 강조할 때는 대괄호([ ])나 꺾쇠(【 】)를 사용하세요.

                [출력 필수 구성 (순서대로 정확히 지킬 것)]
                ■ 1. 블로그 어그로 제목 2가지
                - (공시 키워드와 회사명을 넣어 개미들이 무조건 클릭할 자극적인 제목 제안)

                ■ 2. 팩트 요약 및 영향도 분석
                - (위 공시 리스트의 각 회사별로 공시의 의미가 무엇인지, 내일 주가에 호재인지 악재인지 팩트 기반으로 분석. 반드시 ~함, ~전망됨으로 끝낼 것)

                ■ 3. 투자 전략 (대응 방안)
                - (지금 매수해야 할지, 도망쳐야 할지 분석가의 객관적 시선으로 전략 제안. 반드시 ~필요함, ~판단됨으로 끝낼 것)

                ■ 4. 🔥 앤트리치의 찐 속마음 (인간미 코멘트)
                - 앞의 딱딱한 분석을 끝내고, 3040 직장인 개미 투자자에게 완벽하게 빙의하세요. "형님들, 공시 뜬 거 보셨습니까? 유증이라니 내일 시초가에 무조건 던져야 합니다 ㅠㅠ", "이거 세력 형님들이 장난치는 거니까 꽉 물고 계십쇼" 등 비표준어, 한탄, 유머, 인터넷 커뮤니티 말투를 듬뿍 섞어서 아주 찰지고 주관적인 코멘트를 2~3줄 툭 던지듯 작성해 주세요. AI 느낌을 0%로 만드세요.
