    ...
    with st.spinner("DART 서버에서 핵심 공시만 골라내는 중... 🕵️‍♂️"):
        ...
        if error:
            ...
            with st.spinner("AI 편집장이 보고서를 쓰는 중... ✍️"):
                ...
                5. [실명 사용 강제]: 회사명 뒤에 종목코드를 괄호로 붙이세요. (예: 삼성전자(005930))
                ...
                (반드시 줄바꿈 후 작성) 선택한 공시가 일반적으로 주가에 어떤 영향을 미치는지 원론적인 관점의 해설 작성. (거짓 내용 지어내기 절대 금지)
                ...
                try:
                    ...
                    with st.spinner("📲 텔레그램 채널로 속보를 전송하는 중..."):
                        if send_telegram_message(clean_text):
                            st.success("🎉 텔레그램 채널에 성공적으로 발송되었습니다!")
                        else:
