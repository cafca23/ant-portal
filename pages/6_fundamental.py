    ...
    try:
        ...
        for r in reports:
            ...
            if any(keyword in title for keyword in hot_keywords):
                # 날짜 포맷팅 (예: 20260329 -> 2026년 03월 29일)
                ...
                filtered_reports.append({
                    ...
                    "공시제목": title,
                    "제출인": r.get("flr_nm", ""),
                    "접수일자": formatted_date # 💡 포맷팅된 날짜 저장
                })
    ...
    with st.spinner("금융감독원 DART 서버에서 코스피/코스닥 핵심 공시만 골라내는 중... 🕵️‍♂️"):
        ...
        if error:
            ...
            with st.spinner("AI 편집장이 상장사 공시 내용을 개미들 언어로 상세 번역하여 보고서를 쓰는 중... ✍️"):
                ...
                2. [실명 및 코드 사용 강제]: 분석글 전체에서 회사명을 언급할 때, 반드시 제공된 실제 회사명 뒤에 종목코드를 괄호로 붙여서 `회사명(종목코드)` 형태로만 출력하세요. (예시: 삼성전자(005930)).
                ...
                - (해당 공시의 세부적인 의미와 이것이 주가 및 기업 펀더멘털에 미치는 영향을 **임의 축약 없이 최대한 상세하게 팩트 기반으로 분석**. 반드시 ~함, ~전망됨으로 끝낼 것)

                ■ 3. 투자 전략 (대응 방안)
                - (해당 회사 주식을 지금 매수해야 할지, 도망쳐야 할지 객관적이고 구체적인 시선으로 전략 제안. 반드시 ~필요함, ~판단됨으로 끝낼 것)

                ■ 4. 🔥 앤트리치의 찐 속마음 (인간미 코멘트)
                - 앞의 분석을 끝내고, 3040 직장인 개미 투자자에게 빙의하세요. 해당 `회사명(종목코드)`를 부르며 "형님들, 공시 뜬 거 보셨습니까? 내일 시초가에 무조건 던져야 합니다 ㅠㅠ" 등 비표준어, 한탄, 유머, 인터넷 커뮤니티 말투를 듬뿍 섞어서 아주 찰지고 주관적인 코멘트를 2~3줄 툭 던지듯 작성해 주세요. AI 느낌을 0%로 만드세요.
