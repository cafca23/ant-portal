...
if st.button("🔥 실시간 속보 스캔 & 텔레그램 발사!", use_container_width=True):
    ...
    if error:
        ...
        with st.spinner("AI 편집장이 보고서를 쓰는 중... ✍️"):
            ...
            5. [기업명 표기 강제]: 제공된 데이터의 [타겟종목]에 적혀있는 '실제 기업의 이름'을 반드시 출력하세요. 기업명 뒤에는 종목코드를 괄호로 붙이세요. (작성 예시: 삼성전자(005930))
            ...
            # 💡 이 부분이 아까 날아갔던 AI 실행 및 전송 엔진입니다!
            try:
                response = model.generate_content(prompt)
                clean_text = response.text.replace('*', '')
                clean_text = re.sub(r'[\U00010000-\U0010ffff]', '', clean_text) # 이모지 완벽 제거
