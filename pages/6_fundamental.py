...
if st.button("🔥 실시간 돈 되는 상장사 공시 스캔하기", use_container_width=True):
    with st.spinner("금융감독원 DART 서버에서 코스피/코스닥 핵심 공시만 골라내는 중... 🕵️‍♂️"):
        ...
        if error:
            ...
            # 💡 [데이터 주입 수정] AI가 헷갈리지 않게 '타겟종목'이라고 명확히 짚어줍니다.
            reports_text = ""
            ...
            with st.spinner("AI 편집장이 상장사 공시 내용을 개미들 언어로 상세 번역하여 보고서를 쓰는 중... ✍️"):
                # 💡 [프롬프트 핵심 수정] '회사명'이라는 단어 출력 자체를 물리적으로 금지시킵니다!
                prompt = f"""
                ...
                2. [실명 사용 완벽 강제]: 절대로 '회사명', '이 기업', '특정 기업'이라는 단어를 화면에 출력하지 마세요. 데이터에 기재된 [타겟종목]의 '진짜 이름'을 정확히 추출해서 작성하세요. 항상 진짜 이름 뒤에 종목코드를 괄호로 붙이세요. (작성 예시: 경동나비엔(009450), 삼성전자(005930))
                3. [상세 분석 강제 (요약 금지)]: 공시의 세부 조건은 투자 판단에 직결되는 매우 중요한 정보입니다. 임의로 내용을 줄이거나 요약하지 마세요. 공시의 구체적 의미와 파급력을 최대한 길고 상세하게 깊이 있게 분석하세요.
                4. [어투 종결 강제]: 2~4번 항목의 모든 문장 끝은 무조건 "~함", "~됨", "~했음", "~예상됨", "~필요함" 형식의 개조식으로만 작성하세요.
                5. [기호 사용 완전 통제]: 글 전체에 걸쳐서 별표(*) 기호와 이모티콘(이모지)은 단 한 개도 절대 사용하지 마세요. 강조할 때는 대괄호([ ])나 꺾쇠(【 】)를 사용하세요.

                [출력 필수 구성 (순서대로 정확히 지킬 것)]
                ■ 1. 앤트리치 전용 메인 제목 (단 1개만)
                - 작성 공식: [긴급 팩트체크] 진짜이름(종목코드) '공시 종류', 호재일까 악재일까?

                ■ 2. [공시일자: 데이터의 접수일자 기입] 팩트 상세 분석 및 영향도
                - (타겟 회사의 정확한 공시일자를 소제목 괄호 안에 적어주세요.)
                - (타겟종목의 진짜이름을 주어로 사용하여 해당 공시의 세부적인 의미와 이것이 주가 및 기업 펀더멘털에 미치는 영향을 임의 축약 없이 최대한 상세하게 팩트 기반으로 분석. 반드시 ~함, ~전망됨으로 끝낼 것)

                ■ 3. 투자 전략 (대응 방안)
                - (해당 회사 주식을 지금 매수해야 할지, 도망쳐야 할지 객관적이고 구체적인 시선으로 전략 제안. 반드시 ~필요함, ~판단됨으로 끝낼 것)

                ■ 4. 🔥 앤트리치의 찐 속마음 (인간미 코멘트)
                - 앞의 분석을 끝내고, 3040 직장인 개미 투자자에게 빙의하세요. 해당 타겟종목의 진짜이름을 부르며 "형님들, 경동나비엔 공시 뜬 거 보셨습니까? 내일 시초가에 무조건 던져야 합니다 ㅠㅠ" 등 비표준어, 한탄, 유머, 인터넷 커뮤니티 말투를 듬뿍 섞어서 아주 찰지고 주관적인 코멘트를 2~3줄 툭 던지듯 작성해 주세요. AI 느낌을 0%로 만드세요.
