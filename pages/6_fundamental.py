...
def get_hot_dart_reports():
    ...
    try:
        ...
        for r in reports:
            # 💡 [핵심 추가] 법인구분(corp_cls) 필터링: Y(코스피) 또는 K(코스닥) 상장사만 통과!
...
if st.button("🔥 실시간 돈 되는 상장사 공시 스캔하기", use_container_width=True):
    with st.spinner("금융감독원 DART 서버에서 코스피/코스닥 핵심 공시만 골라내는 중... 🕵️‍♂️"):
        ...
        if error:
            ...
            st.info("현재 코스피/코스닥 상장사 중에 뜬 굵직한 호재/악재 공시가 없습니다. 내일 다시 스캔해 보세요!")
        else:
            st.success(f"✅ 총 {len(hot_reports)}건의 돈 되는 상장사 공시를 포착했습니다!")
            
            reports_text = ""
            for i, r in enumerate(hot_reports):
                reports_text += f"- [실제 회사명: {r['회사명']}] {r['공시제목']}\n"
            ...
            with st.spinner("AI 편집장이 상장사 공시 내용을 개미들 언어로 번역하여 보고서를 쓰는 중... ✍️"):
                ...
                아래는 최근 3일간 코스피/코스닥 상장사들이 발표한 공시 리스트입니다.
                ...
                2. [실명 사용 강제]: 분석글을 쓸 때 '특정 기업', '[실제 회사명: ]' 같은 빈칸이나 대명사를 절.대.로 출력하지 마세요. 타겟으로 고른 회사의 **'진짜 이름(예: 삼성전자, 카카오)'**을 문장에 명확하게 적으세요.
                3. [어투 종결 강제]: 2~4번 항목의 모든 문장 끝은 무조건 "~함", "~됨", "~했음", "~예상됨", "~필요함" 형식의 개조식으로만 작성하세요.
                ...
                - 작성 공식: [긴급 팩트체크] 진짜회사명 '공시 종류', 호재일까 악재일까?
                - (예시: [긴급 팩트체크] 삼성전자 '유상증자', 호재일까 악재일까?)

                ■ 2. 팩트 요약 및 영향도 분석
                - (타겟으로 선정한 진짜 회사명을 주어로 하여, 내일 주가에 호재인지 악재인지 팩트 기반으로 분석. 반드시 ~함, ~전망됨으로 끝낼 것)

                ■ 3. 투자 전략 (대응 방안)
                - (지금 매수해야 할지, 도망쳐야 할지 객관적 시선으로 전략 제안. 반드시 ~필요함, ~판단됨으로 끝낼 것)

                ■ 4. 🔥 앤트리치의 찐 속마음 (인간미 코멘트)
                - 앞의 딱딱한 분석을 끝내고, 3040 직장인 개미 투자자에게 완벽하게 빙의하세요. 타겟 회사명을 직접 부르며 "형님들, 이 회사 공시 뜬 거 보셨습니까? 내일 시초가에 무조건 던져야 합니다 ㅠㅠ" 등 비표준어, 한탄, 유머, 인터넷 커뮤니티 말투를 듬뿍 섞어서 아주 찰지고 주관적인 코멘트를 2~3줄 툭 던지듯 작성해 주세요. AI 느낌을 0%로 만드세요.
