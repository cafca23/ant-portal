...
st.title("🏛️ J.F2.A 앤트리치 시황 브리핑 공장 V4.4 (Pro Edition)")
...
if st.button(btn_text, use_container_width=True):
    with st.spinner(f"데이터와 뉴스를 싹쓸이하여 {target_market} 전문 전략 보고서를 작성 중입니다... ☕ (캐싱 적용으로 약 10초 소요)"):
        ...
        # 💡 [핵심 패치] 검색 순위 표기(X위) 제거 및 해시태그 정규화 완벽 적용
        prompt = f"""
        ...
        2. [팩트 강조]: 핵심 팩트나 단어를 강조할 때 대괄호([ ])나 꺾쇠(【 】) 같은 특수기호로 감싸지 말고 텍스트만 깔끔하게 작성하세요.
        ...
        5. [검색 랭킹 자연스럽게 녹이기]: 한국 증시의 경우 제공된 [네이버 실시간 검색 상위 종목]에 포함된 종목이 특징주나 이슈에 있다면 브리핑 시 최우선으로 자연스럽게 언급하되, 종목명 옆에 '(검색 X위)' 같은 순위 표기 꼬리표는 절대 달지 마세요.
        ...
        7. 블로그용 핵심 태그
        - "블로그용 해시태그" 같은 설명 문구는 절대 쓰지 말고, 각 단어 앞에 무조건 '#' 기호를 붙여서 쉼표(,) 없이 빈칸(스페이스바)으로 한 칸씩만 띄워서 딱 10개 나열할 것. (예: #미국증시 #주식투자 #시황분석 #나스닥 #코스피)
        """
