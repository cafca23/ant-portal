...
st.title("🏛️ J.F2.A 앤트리치 시황 브리핑 공장 V3.1 (인간미 탑재)")
...
if st.button(btn_text, use_container_width=True):
    with st.spinner(f"데이터와 뉴스를 싹쓸이하여 {target_market} 전략 보고서를 작성 중입니다... ☕ (약 15초 소요)"):
        ...
        # 💡 [AI 편집장 지시사항] - 인간미 코멘트 추가 및 별표 원천 차단 룰 적용
        prompt = f"""
        당신은 기업의 전문 투자 분석가이자 주식 블로그 '앤트리치'의 수석 에디터입니다. 
        오늘 수집된 팩트 데이터를 바탕으로, 블로그에 업로드할 시황 브리핑을 작성해 주세요.
        ...
        [🚨 매우 중요한 작성 규칙 - 절대 엄수]
        1. [어투 종결 강제]: 1번~5번 항목까지의 문장 끝은 무조건 "~함", "~됨", "~했음", "~예상됨", "~필요함" 형식의 개조식으로만 작성하세요. 
        2. [기호 사용 완전 통제]: 글 전체에 걸쳐서 별표(*) 기호는 단 한 개도 사용하지 마세요. 목록은 하이픈(-)만, 강조는 대괄호([ ])나 꺾쇠(【 】)만 사용하세요.

        [출력 필수 구성]
        1. [보고서 제목] : 블로그에 적합하면서 검색에 강한 제목 3가지 추천 (예: [국장 마감] 기관 쌍끌이 특징주 요약 & 내일 장 시초가 공략 전략)
        2. [금일 시장 개요 보고] : 시장의 전체적인 흐름 1~2줄 요약.
        3. [지수 및 특징주 동향 분석] : 왜 올랐는지/떨어졌는지 주요 원인 분석. 
        4. [앤트리치의 대응 전략 및 인사이트] : {insight_focus}에 대한 전문적인 분석 및 향후 액션 플랜 제안.
        5. [마법의 1인치: D-Day 캘린더] : 제공된 일정 데이터를 바탕으로 핵심 일정만 요약.
        6. 🔥 [앤트리치의 찐 속마음 (인간미 코멘트)] : 분위기를 확 바꿔서 3040 직장인 개미 투자자에게 빙의해 작성하세요. 앞의 딱딱한 개조식을 버리고, "형님들, 오늘 수급 보니까 내일 무조건 ~~ 탑승해야 합니다", "물린 분들 오늘 밤 미장 보면서 기도합시다 ㅠㅠ" 등 비표준어, 한탄, 유머, 인터넷 커뮤니티 말투를 섞어서 아주 찰지고 주관적인 코멘트를 2~3줄 툭 던지듯 작성해 주세요. AI가 쓴 기계적인 느낌이 1%도 나면 안 됩니다.
        7. [블로그용 핵심 태그] : 핵심 키워드 10~15개 (해시태그 기호 # 만 사용)
        8. [🎨 이미지 AI용 주문서]: 맨 마지막 줄에 정확히 아래 문장만 그대로 출력해 주세요.
           "이 브리핑 내용에 맞는 이미지 3개 만들어 줘. 합치지 말고 무조건 1컷당 1개의 이미지 파일로 분리해서 생성해 줘."
        """
        ...
        try:
            ...
            # 💡 [핵심] 파이썬에서 물리적으로 별표(*)를 모두 지워버립니다! AI가 실수해도 여기서 걸러집니다.
