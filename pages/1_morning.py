...
st.title("🏛️ J.F2.A 앤트리치 시황 브리핑 공장 V3.1")
...
if st.button(btn_text, use_container_width=True):
    with st.spinner(f"데이터와 뉴스를 싹쓸이하여 {target_market} 전략 보고서를 작성 중입니다... ☕ (약 15초 소요)"):
        ...
        [🚨 매우 중요한 작성 규칙 - 절대 엄수]
        1. [어투 종결 강제]: 모든 문장의 끝은 무조건 "~함", "~됨", "~했음", "~예상됨", "~필요함" 형식으로 철저하게 끊어지는 명사형 개조식으로만 작성하세요. 줄글이나 대화체는 절대 금지입니다.
        2. [기호 사용 통제]: 글 전체에 걸쳐서 별표(*) 기호는 단 한 개도 사용하지 마세요. 목록을 만들 때는 오직 하이픈(-)만 사용하세요.
        3. [팩트 강조]: 핵심 팩트(수치, 종목명 등)를 강조할 때는 별표(*) 대신 대괄호([ ])나 꺾쇠(【 】)를 사용하세요.
        ...
        5. [마법의 1인치: D-Day 캘린더] : 제공된 '4. 네이버 실시간 증시 일정' 팩트 데이터를 바탕으로 오늘 또는 이번 주 핵심 일정만 요약 보고.
        6. [블로그용 핵심 태그] : 핵심 키워드 10~15개 (해시태그 기호 # 만 사용)
        7. [🎨 이미지 AI용 주문서]: 맨 마지막 줄에 정확히 아래 문장만 그대로 출력해 주세요.
