...
with col6:
    try:
        ...
        telegram_data['WTI'] = f"${wti_price:.2f} ({wti_pct:+.2f}%)"
    except Exception as e:
        st.metric(label="🛢️ WTI 국제 유가", value="불러오기 실패")
        telegram_data['WTI'] = "데이터 수집 실패"
...
with col7:
    try:
        ...
        telegram_data['금'] = f"${gold_price:,.1f} ({gold_pct:+.2f}%)"
    except Exception as e:
        st.metric(label="🥇 국제 금값", value="불러오기 실패")
        telegram_data['금'] = "데이터 수집 실패"
...
with col8:
    try:
        ...
        telegram_data['BTC'] = f"${btc_price:,.0f} ({btc_pct:+.2f}%)"
    except Exception as e:
        st.metric(label="🪙 비트코인", value="불러오기 실패")
        telegram_data['BTC'] = "데이터 수집 실패"
...
# ==========================================
# [최하단] 텔레그램 전송 및 블로그 이동 버튼
# ==========================================
col_btn1, col_btn2, col_btn3 = st.columns([1, 2, 1])

with col_btn2:
    st.link_button("🏠 앤트리치 블로그로 돌아가기 👉", 
    ...
    if st.button("📲 앤트리치 텔레그램 채널로 시황 전송하기", type="primary", use_container_width=True):
        ...
        # 텔레그램에 보낼 메시지 예쁘게 포장하기
        msg = f"📊 <b>[앤트리치 3대 심리 & Macro 현황판]</b>\n\n"
        msg += f"🇺🇸 미국 VIX : {telegram_data.get('VIX')}\n"
        msg += f"🦅 미국 CNN : {telegram_data.get('CNN')}\n"
        msg += f"🐯 한국 V-KOSPI : {telegram_data.get('VKOSPI')}\n\n"
        msg += f"💵 환율(USD/KRW) : {telegram_data.get('환율')}\n"
        msg += f"🏛️ 미 10년물 금리 : {telegram_data.get('국채금리')}\n"
        msg += f"🛢️ WTI 유가 : {telegram_data.get('WTI')}\n"
        ...
        with st.spinner("텔레그램 채널로 전송 중입니다..."):
            try:
                ...
                st.error("🚨 텔레그램 API 키가 설정되지 않았습니다. .streamlit/secrets.toml 파일을 확인해 주세요.")
            except Exception as e:
