...
# ==========================================
# ⚙️ 2단계: AI 편집장 브리핑 작성 (체류시간 3종 세트 탑재)
# ==========================================
def generate_morning_briefing(co_buy, after_hours):
    ...
    [작성 가이드라인]
    1. 도입부: 활기찬 아침 인사와 전날 증시 분위기 2줄 요약.
    2. 본문: '🔥 세력이 찜한 쌍끌이 TOP 5', '🚀 밤사이 터진 시간외 급등 TOP 5' 소제목을 달고 불릿 포인트로 정리.
    
    3. ✨ [마법의 1인치 ①] 💡 앤트리치의 시선 (섹터 힌트): 데이터의 공통점을 분석하고, 대장주 상승으로 인해 오늘 수혜를 볼 수 있는 '관련 중소형주 섹터나 서브 테마(낙수효과)'를 1~2개 넌지시 힌트로 던져주세요. (단, 특정 종목 직접 추천은 금지)
    
    4. ✨ [마법의 1인치 ②] 📅 오늘의 D-Day 캘린더: 오늘 또는 이번 주 시장에 영향을 줄 만한 거시경제 일정이나 주요 이벤트를 1~2줄로 짧게 요약해 주세요.
    
    5. ✨ [마법의 1인치 ③] 📺 앤트리치의 영상 픽 (유튜브 요약 코너 세팅): 평소 에디터가 직접 분석하는 주식 유튜브 요약본이 들어갈 빈칸 틀을 아주 예쁘게 만들어 주세요. 
       - 예시 형태: "▶️ **[오늘의 핵심 유튜브 요약]** \n (에디터님, 여기에 직접 분석하신 유튜브 3줄 요약을 넣어주세요!)"
       
    6. ✨ [만화 주문서 통합 작성]: 맨 마지막에 구분선(---)을 긋고, 나노바나나 등 이미지 AI용 [🎨 이미지 AI용 만화 주문서]를 한글로 작성해 주세요.
       - 필수 지시사항: "중요: 만화 이미지를 합치지 말고, 무조건 1컷당 1개의 이미지 파일로 분리해서 생성해 줘."
       - 4컷 만화 스토리, 장면 묘사, 3040 개미가 공감할 리얼한 대사를 포함하세요.
       
    7. 금지사항: 앞뒤에 ```markdown 기호는 절대 넣지 마세요.
    """
