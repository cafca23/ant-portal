...
st.write("10% 이상 움직인 종목 중 **'네이버 실시간 검색 랭킹'**이 높은 주도주를 우선 스캔하여 보고서를 작성합니다.")
st.divider()
...
# 💡 [핵심 패치 1] 네이버 증권 '검색 상위 종목' 실시간 스크래핑 함수
@st.cache_data(ttl=60, show_spinner=False) # 1분마다 갱신
def get_naver_search_ranks():
            ...
            for row in rows:
                ...
                if len(cols) >= 2:
                    ...
                    if name_tag and rank_tag.text.strip().isdigit():
                        ...
                        ranks[name] = rank # 딕셔너리에 {종목명: 순위} 저장
...
def run_scanner(market, mover_type="gainers"):
    ...
    # 💡 스캔 시작 전 네이버 검색 랭킹을 미리 가져옵니다.
    search_ranks = get_naver_search_ranks()
        ...
        try:
            ...
            for stock in quotes:
                ...
                if (mover_type == "gainers" and change_pct >= 10.0) or (mover_type == "losers" and change_pct <= -10.0):
                    # 미국 주식은 네이버 검색어 랭킹이 없으므로 기본값(999) 부여
                    results.append({"symbol": symbol, "name": name, "change": change_pct, "is_us": True, "search_rank": 999})
        except:
            pass
                        ...
                        if name_tag:
                            ...
                            try:
                                ...
                                if (mover_type == "gainers" and pct >= 10.0) or (mover_type == "losers" and pct <= -10.0):
                                    # 💡 [핵심 패치 2] 실시간 검색어 순위 매칭 (순위권 밖이면 999위로 설정)
                                    rank = search_ranks.get(name, 999)
                                    results.append({"symbol": name, "name": name, "change": pct, "is_us": False, "search_rank": rank})
                            except:
                                pass
            except:
                continue
    ...
    # 💡 [핵심 패치 3] 정렬 로직 변경: 1순위(검색어 랭킹 높을수록), 2순위(등락률 절댓값 클수록)
    results = sorted(results, key=lambda x: (x['search_rank'], -abs(x['change'])))
    
    return results[:10] # 상위 10개까지 여유있게 보여주기

col1, col2 = st.columns(2)
with col1:
    if st.button("🚀 10% 이상 폭등주 스캔", use_container_width=True):
        with st.spinner("야후/네이버 금융 스캔 및 검색어 랭킹 융합 중..."):
...
if st.session_state.scan_results:
    st.divider()
    st.subheader("🎯 스캔 완료! (🔥실시간 검색 랭킹 우선 정렬)")
    
    options = {}
    for item in st.session_state.scan_results:
        # 💡 [핵심 패치 4] 드롭다운 메뉴에 검색어 순위를 직관적으로 표시
        if item['search_rank'] < 999:
            rank_str = f"🔥검색 {item['search_rank']}위"
        else:
            rank_str = "📈순위 밖"
            
        label = f"[{rank_str}] {item['name']} / 변동: {item['change']:+.2f}%"
        options[label] = item
        
    selected_label = st.selectbox("스캔된 종목 (가장 위쪽이 시장 주도주입니다):", list(options.keys()))
