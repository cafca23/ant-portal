    ...
    if st.button("✍️ [급등락 심층 보고서] 출력", type="primary", use_container_width=True):
        if not target_stock:
          ...
          # 💡 [핵심] 이미지 대사 삽입 지시 및 기업 소개 추가
                ...
                2. [어투 종결 강제]: 문장의 끝에 절대 "~습니다", "~입니다", "~해요"를 사용하지 마세요. 무조건 "~함", "~됨", "~했음", "~예상됨", "~필요함" 형식으로 철저하게 끊어지는 명사형 개조식으로만 작성하세요. (예: 상승 마감함, 관망세가 짙어짐, 주의가 필요함)
                3. [특수기호 제한]: 강조나 글머리 기호 목적으로 별표(*) 기호를 절대 사용하지 마세요. 글머리 기호가 필요하다면 하이픈(-)을 사용하세요.
                ...
                try:
                    ...
                    # 💡 파이썬 단에서도 혹시 모를 * 기호를 강제로 싹 지워버립니다.
