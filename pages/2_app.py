    ...
    if target_stock == "":
        ...
        with st.spinner(f"[{target_stock}] 관련 데이터를 취합하여 공식 보고서를 작성 중입니다... ✍️"):
            ...
            if not target_news_titles:
                ...
                [기본 구성] 10컷 만화 대본
                - 10컷 분량의 대본 작성. 
                - 각 컷별로 이미지 생성 지시문(영어로, 대사가 이미지 말풍선에 포함되도록 명령)과 한글 대사 필수 포함.
                - 대사 부분에 화자 이름(예: 개미1)은 절대 적지 말고 큰따옴표 안의 대사만 작성할 것.

                1. (보고서 형식의 깔끔한 제목 2가지)
                2. 회사 소개 : (해당 기업의 비즈니스 모델을 간결한 개조식으로 요약)
                3. 재무제표 요약 : (알려진 최신 재무 상태나 실적 동향을 팩트 위주로 간략히 보고)
