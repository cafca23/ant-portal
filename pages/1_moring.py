...
if st.button(btn_text, use_container_width=True):
    with st.spinner(f"데이터와 뉴스를 싹쓸이하여 {target_market} 브리핑을 작성 중입니다... ☕ (약 15초 소요)"):
        ...
        [🚨 매우 중요한 작성 규칙 및 양식]
        1. [어투 종결 강제]: 문장의 끝에 절대 "~습니다", "~입니다", "~해요"를 사용하지 마세요. 무조건 **"~함", "~됨", "~했음", "~예상됨", "~필요함"** 형식으로 철저하게 끊어지는 명사형 개조식으로만 작성하세요. (예: 상승 마감함, 관망세가 짙어짐, 주의가 필요함)
        2. [팩트 강조]: 보고서의 핵심 팩트(수치, 증감률, 주요 특징주 이름, 핵심 원인 등)는 반드시 **굵은 글씨(마크다운)**로 처리하여 가독성을 높이세요.
        3. [구조화]: 줄글은 배제하고 기호(-, *)를 적극 활용하여 팩트만 남은 완벽한 개조식 보고서를 작성하세요.

        [출력 필수 구성]
        1. [보고서 제목] : 블로그에 적합하면서도 전문적인 일일보고 형태의 제목 3가지 추천 (예: [일일보고] O월 O일 미 증시 동향 및 주요 특징주 분석)
        2. [금일 시장 개요] : 시장의 전체적인 마감 흐름을 팩트 위주로 1~2줄 요약 보고. (반드시 ~함, ~됨 으로 끝낼 것)
        3. [지수 및 주요 종목 동향 분석] : 수집된 지수와 대장주 데이터를 바탕으로 시장이 왜 올랐는지/떨어졌는지 주요 원인을 간결하게 보고.
        4. [대응 전략 및 인사이트 보고] : {insight_focus}에 대한 전문적인 분석 및 향후 액션 플랜 제안. (반드시 ~예상됨, ~필요함 등으로 끝낼 것)
        5. [블로그용 해시태그] : 관련된 핵심 키워드 10~15개 (쉼표로 구분)
        6. [🛡️ 면책 조항] : (※ 본 포스팅은 정보 제공을 위한 참고용일 뿐, 투자의 최종 판단과 책임은 투자자 본인에게 있음.)
